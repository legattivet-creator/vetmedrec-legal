TERMS OF USE – VETMEDREC
Last updated: January 12, 2026
1. ACCEPTANCE OF TERMS
By using the VetMedRec application, the user confirms that they have read, understood and fully agreed to these Terms of Use.
If the user does not agree with any provision, they must immediately stop using the application.
2. ABOUT THE APPLICATION
VetMedRec is an organizational and informational application designed to store, organize and access information related to pets’ health and routine, including medical records, documents, logs, reminders and histories.
The application does not replace, under any circumstances, in-person veterinary consultations, clinical diagnoses, professional evaluations or veterinary medical advice.
Information stored in the application must not be used as the sole basis for clinical, therapeutic or emergency decisions.
3. USER ACCOUNT
Certain features may require account creation.
Users are solely responsible for:
safeguarding their login credentials;
ensuring that all provided information is accurate and up to date;
all activities performed through their account.
4. SERVICE NATURE AND DISCLAIMER OF WARRANTIES
VetMedRec is provided “as is”, without any warranty of uninterrupted operation, error-free performance or compatibility.
Users acknowledge that service interruptions, delays or failures may occur due to technical issues or third-party services.
5. SCHEDULE, REMINDERS AND ALERTS
Scheduling, reminder and alert features are supportive tools only, intended to assist user organization.
Users acknowledge that:
notifications depend on device permissions, operating systems, connectivity and third-party services;
VetMedRec does not guarantee notification delivery or visibility.
VetMedRec shall not be liable for:
forgotten events;
notification failures or delays;
consequences resulting from missed appointments, treatments or medications.
6. USER-GENERATED CONTENT
All content submitted to the application, including texts, images, videos and documents, is the sole responsibility of the user.
Users declare that they hold the necessary legal rights over submitted content and fully indemnify VetMedRec against any related claims.
7. INFORMATION SHARING
Data sharing between users occurs only with explicit consent.
Users may revoke sharing permissions at any time.
VetMedRec is not responsible for how third parties use information shared with user authorization.
8. PAID PLANS
The application may offer paid subscription plans with additional features.
Subscription to paid plans does not imply any guarantee of performance, results, service continuity or error-free operation, nor does it modify liability limitations.
9. LIMITATION OF LIABILITY
To the fullest extent permitted by law, VetMedRec shall not be liable for direct or indirect damages, moral damages, loss of profits, data loss or any other losses.
Any potential liability shall be limited to the amount actually paid by the user in the 12 (twelve) months preceding the event giving rise to the claim.
10. SUSPENSION AND TERMINATION
VetMedRec may suspend or terminate accounts that violate these Terms, without prior notice, when necessary.
Users may request account termination at any time.
11. GOVERNING LAW AND JURISDICTION
These Terms of Use are governed by the laws of the Portuguese Republic.
The Lisbon District Court, Portugal, shall have exclusive jurisdiction to resolve any disputes.
12. OWNERSHIP
The application is currently operated by a natural person and may be transferred to a legal entity in the future without requiring renewed consent.
13. CONTACT
Emerson Legatti
suportevetmedrec@gmail.com
